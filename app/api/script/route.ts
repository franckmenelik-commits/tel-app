...
// POST /api/script — Génère un script vidéo à partir d'un InsightCard
...
async function callLLM(prompt: string): Promise<string> {
  // N3 — Anthropic Claude (best for creative writing)
  if (process.env.ANTHROPIC_API_KEY) {
    try {
      ...
      console.warn('[script] N3 indisponible:', err instanceof Error ? err.message : err)
    }
  }
  ...
  if (process.env.MISTRAL_API_KEY) {
    try {
      ...
      console.warn('[script] N2 indisponible:', err instanceof Error ? err.message : err)
    }
  }
...
export async function POST(request: Request) {
  try {
    ...
    const prompt = buildScriptPrompt(insight)
